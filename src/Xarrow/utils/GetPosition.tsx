...
export const getPosition = (xProps: useXarrowPropsResType, mainRef: React.MutableRefObject<any>) => {
  ...
  if (path === 'grid') {
    if (gridRadius && !(cpx1 === cpx2 && cpy1 === cpy2)) {
      if (x2 === cpx2 && y2 === cpy2) {
        // 2 lines, 1 corner
        const roundedEdge = getRoundedEdge(gridRadius, { x: x1, y: y1 }, { x: cpx1, y: cpy1 }, { x: x2, y: y2 });
        arrowPath = `M ${x1} ${y1} ${roundedEdge} L ${x2} ${y2}`;
      } else {
        // 3 lines, 2 corners
        const roundedEdge1 = getRoundedEdge(gridRadius, { x: x1, y: y1 }, { x: cpx1, y: cpy1 }, { x: cpx2, y: cpy2 });
        const roundedEdge2 = getRoundedEdge(gridRadius, { x: cpx1, y: cpy1 }, { x: cpx2, y: cpy2 }, { x: x2, y: y2 });
